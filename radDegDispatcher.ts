/* Radians ↔ Degrees Game Logic */
interface RDQuestion {
  prompt: string;       // TeX prompt (eg 30° or -\frac{4\pi}{3})
  answerValue: number;  // numeric value (degrees or radians) for comparison
  expectsRad: boolean;  // true if user should answer in radians, else degrees
}

const RD_TIMEOUT = 15_000;
...
/* --- helpers --- */
function fracToTeX(n: number, d: number) {
  if (d === 1) return n === 0 ? "0" : `${n === 1 ? '' : n}\\pi`;
  return `${n === 1 ? '' : n}\\pi/${d}`;
}
function fracToNumeric(n: number, d: number) { return (n * Math.PI) / d; }

function degToRadTeX(deg: number) {
  const rad = (deg * Math.PI) / 180;
  // match to known frac table
  for (const {num, den} of RAD_FRACS) {
    if (Math.abs(rad - fracToNumeric(num, den)) < 1e-6) return fracToTeX(num, den);
    if (Math.abs(rad + fracToNumeric(num, den)) < 1e-6) return '-' + fracToTeX(num, den);
  }
  return rad.toFixed(3);
}

function radFracToDeg(num: number, den: number) { return (num * 180) / den; }

/* --- parse user answer --- */
function parseRad(str: string): number | null {
...
function parseDeg(str: string): number | null {
...
let currentQ: RDQuestion | null = null;
...
form.addEventListener('submit',evt=>{
  ...
  if(Math.abs(parsed- currentQ.answerValue) < 0.01){
    score++;updateScore(); nextQuestion();
  } else {
    score--; updateScore(); if(score<0) finish('Score below 0');
  }
});
...
function startTimer(){
  ...
  timerId = window.setInterval(()=>{rem--; timerDiv.textContent=`Time ${rem}s`; if(rem===0){clearInterval(timerId!); finish('Time up');}},1000);
}

function renderQuestion(tex:string){
...
function finish(msg:string){
  if(timerId) clearInterval(timerId);
  if(score>highScore){highScore=score; localStorage.setItem('rdHighScore',String(highScore));}
  if(gameOverScreen){finalScoreP.textContent=`Your score: ${score}`; gameOverScreen.classList.remove('hidden');} else {alert(`Game over: ${msg}\nFinal score: ${score}`); window.location.reload();}
}
