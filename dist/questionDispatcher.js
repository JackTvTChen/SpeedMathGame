...
const ANSWER_TIMEOUT = 20000; // 10 s
...
let timerId = null;
let fileReader;
const pool = Array.from({ length: 31 }, () => []); // 1‑based
const recentSeen = JSON.parse(localStorage.getItem("recentQs") ?? "[]");
const recentSet = new Set(recentSeen);
/* ── DOM refs ────────────────────────────────────────────────── */
...
(async function init() {
    ...
    const SKIP = rngByLevel[0]() % 100000;
...
async function nextQuestion() {
    ...
    while (pool[lvl].length < BUFFER_SIZE) {
        ...
        if (rec.level >= 1 && rec.level <= 30)
            pool[rec.level].push(rec);
    }
