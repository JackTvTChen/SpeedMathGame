...
-------------------------------------------------*/
const ANSWER_TIMEOUT = 15000; // 15 seconds per question
...
function classifyDifficulty(op, deg) {
    ...
    let bonus = 0;
    if (["sec", "csc", "cot"].includes(op))
        bonus += 1;
    if (isUndefined(op, canonical))
        bonus += 1;
    if (deg < 0)
        bonus += 0.5;
    return base + bonus;
